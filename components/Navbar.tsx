...
const Navbar = ({ darkMode, setDarkMode }: NavbarProps) => {
  const [showNav, setShowNav] = useState(false); //mobile navigation menu
  ...
  const navigate = (id : sectionID) => {
    scrollToSection(id);
    setShowNav(false) //calls scrollToSection(id) to scroll to the section, and closes the mobile navigation menu (setShowNav(false)
  }

  //Sets up an IntersectionObserver to detect when a section enters the viewport. This allows:The activeSection to update whenever a section is 20% visible.  Cleanup when the component unmounts, removing the observer.
  ...
  //Responsive styles like md:px-14 for larger screens. Dark mode classes: dark:bg-gray-900 makes the background gray when dark mode is active.
  return (
    <header
      id="nav"
    ...
    >
      <nav className="flex w-full flex-wrap items-center justify-between">
        ...
        />
        {/* Conditionally changes its shape (from three lines to an "X") based on showNav. */}
        ...
        {/* If showNav is true, display the mobile navigation menu. */}
        {/* Displayed as a list of items (li tags) with each linked to a section. Each item has a conditional active class if it corresponds to activeSection. */}
        <div
          className={`${
            showNav ? "" : "hidden"
